...
class LeidenPipelineGUI:
    ...
    def run_pipeline(self, config):
        ...
        try:
            ...
            # Run four algorithms: Louvain (baseline), Leiden, Infomap, and MLCC consensus
            comms_louvain = detect_communities(G, method='louvain', random_state=rs)
            n_louvain = len(set(comms_louvain.values()))
            self.log(f"  Louvain : {n_louvain} communities (baseline, Aidira 2025)")
            ...
            self.log("  MLCC (Pho 2021): running 7-step consensus on {Leiden, Infomap}...")
            ...
            self.log(f"  Consensus (MLCC): {n_consensus} communities")
            ...
            # NMI pairwise among all partitions
            try:
                ...
                for i in range(len(pair_labels)):
                    for j in range(i + 1, len(pair_labels)):
                        ...
                        if len(common_users) > 10:
                            ...
                            if nmi_score > 0.5:
                                self.log(f"    -> Kedua partisi menemukan struktur komunitas yang serupa (NMI > 0.5)")
                            else:
            ...
            #  7. Compute ECR 2.0 for all partitions
            ...
            with open(summary_path, 'w', encoding='utf-8') as f:
                ...
                f.write(_row("Metric", labels_out))
                f.write("-" * header_w + "\n")
                f.write(_row_num("Modularity",
                                 [results[l]['quality'].get('modularity', 0) for l in labels_out]))
                f.write(_row_num("Coverage",
                                 [results[l]['quality'].get('coverage', 0) for l in labels_out]))
                f.write(_row_num("Performance",
                                 [results[l]['quality'].get('performance', 0) for l in labels_out]))
